...
async def help(ctx):
    ...
    embed.add_field(name='No Help Info Available!', value='The help command is disabled as there is no need for it currently', inline=False)
            ...
            if message.author.id != self.bot.user.id:  # check for the bot beign the author
                if message.channel.id in self.channel_whitelist:  # check for channel in channel_whitelist
                    if any(r.match(message.content) for r in self.regexes):  # check if any of the regexes are matched
                        ...
                        await self.send("Invalid Method")
...
faq_messages = [
    FAQMessage_factory(bot, ["dream", "dreams"], [r"dream.?s?.?method", ], channels,
                       "Hey, it looks like you mentioned Dream! Unfortunately we cannot use that method because we don't have enough information. But here's some other ideas: {link to a google doc}"),
    FAQMessage_factory(bot, ["theseed", ], [r"seed.is"], channels,
...
async def on_ready():  # after everything is done and stuff get ready to login and respond to gentards
    await bot.change_presence(activity=discord.Game(name='responding to pack.png questions', type=0,
                                                    url='https://packpng.com/faq'))  # sets description and stuff
